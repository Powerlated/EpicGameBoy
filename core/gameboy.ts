...
import { VBMFrame } from './movies/vbm';
import { MBCWithRAM } from './memory/mbc/mbc';
...
export default class GameBoy {
    ...
    vbmInterval = 0;
    playVBM() {
        let mbc = this.bus.ext.mbc;
        if (mbc instanceof MBCWithRAM) {
            mbc.externalRam.forEach((v, i, a) => {
                a[i] = 0xFF;
            });
        }
        ...
        this.vbmInterval = setInterval(() => {
            for (let i = 0; i < 70224;) {
                let took = this.step();
                i += took;

                if (this.vbmFrame >= this.vbmData.length) {
                    console.log("Movie finished");
                    clearInterval(this.vbmInterval);
                    return;
                }

                let d = this.vbmData[this.vbmFrame];

                this.joypad.a = d.a;
                this.joypad.b = d.b;
                this.joypad.start = d.start;
                this.joypad.select = d.select;
                this.joypad.left = d.left;
                this.joypad.right = d.right;
                this.joypad.up = d.up;
                this.joypad.down = d.down;

                if ((d.resetNew || d.resetOld) && !resetted) {
                    resetted = true;
                    this.reset();
                }

                if (!(d.resetNew || d.resetOld)) {
                    resetted = false;
                }
            }
