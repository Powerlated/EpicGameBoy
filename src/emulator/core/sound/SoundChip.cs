namespace DMSharp
{
    ...
    public class SoundChip
    {
        ...
        static double[] widths = new double[] { 0.5, 0, -0.5, -0.75 }; // CORRECT

        bool enabled = false;

        GameBoy gb;
        long clockMain = 0;
        long clockEnvelope1 = 0;
        long clockEnvelope2 = 0;
        long clockEnvelopeNoise = 0;
        long clockPulse1FreqSweep = 0;

        PulseChannel pulse1 = new PulseChannel();
        PulseChannel pulse2 = new PulseChannel();
        WaveChannel wave = new WaveChannel();
        NoiseChannel noise = new NoiseChannel();

        public SoundChip(GameBoy gb)
        {
            this.gb = gb;
        }

        const int CLOCK_MAIN_STEPS = 65536;
        const int CLOCK_ENVELOPE_STEPS = 65536;
            ...
            if (this.clockMain >= CLOCK_MAIN_STEPS)
            {
                ...
                if (this.pulse1.enabled)
                {
                    if (this.pulse1.lengthEnable)
                    {
                        ...
                        {
                            Util.WriteDebug("PULSE 1 length become 0");
                    ...
                    if (this.pulse1.freqSweepTime != 0)
                    {
                        ...
                        if (this.clockPulse1FreqSweep > this.pulse1.freqSweepTime)
                        {
                            ...
                            var diff = freq / (2 ^ this.pulse1.freqSweepShiftNum);
                            if (this.pulse1.freqSweepUp) { freq += diff; } else { freq -= diff; }
        ...
        public void Write(ushort addr, byte value)
        {
            ...
            switch (addr)
            {
                ...
                case 0xFF1E: // NR34
                    ...
                    Util.WriteDebug(this.wave.lengthEnable.ToString());
                    this.wave.Update();
                    break;
        ...
        {
            var i = this.soundRegisters[addr];
